...
def when_teach():
    ...
    def resultt():
        ...
        for a in glob.glob(path):
            photo1 = cv2.imread("train/img_black_01.jpg",0)
            photo_all = cv2.imread("train/img_black_0"+str(int(i))+".jpg",0)            
        ...
        photo2 = cv2.imread("train/img_black_02.jpg",0)
        photo3 = cv2.imread("train/img_black_03.jpg",0)
    ...
    bt4=Button(window,text="result",border=0,bg="white",command=resultt)
    bt4.place(relx=0.061,rely=0.90,relwidth=0.06,relheight=0.08)
