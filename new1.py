...
#######################################################################################################
        #imggg1=Image.open(fla)
        #imggg2 =Image.open(flb)

        imggrayed= cv2.imread("test_images/train/img_black_01.jpg",0)   #i took trained image here ...........
        #print(imggrayed)

        #print(img1) this is mandatory step because assertion 215 failed comes because of path error   
        #img1_gray = cv2.cvtColor(img1 ,cv2.COLOR_BGR2GRAY)
        
        template = cv2.imread("test_images/test/img00.jpg",0)

        w , h = template.shape[::-1]
        res = cv2.matchTemplate(imggrayed ,template,cv2.TM_CCOEFF_NORMED)
